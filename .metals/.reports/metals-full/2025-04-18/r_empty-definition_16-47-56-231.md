...
def handleUserInput(input: String): Option[String] = {
  parseInput(input).flatMap {
    ...
    case "indian" =>
        ...
        Some(s"Indian food: ${facts(Random.nextInt(facts.size))}")
...
- Base for miso soup and marinades""".stripMargin)
    ...
    case "etiquette" =>
        ...
        Some(s"Dining etiquette: ${tips(Random.nextInt(tips.size))}")
    ...
    case "recipe" =>
        ...
        Some(s"Try this recipe: ${recipes(Random.nextInt(recipes.size))}")
